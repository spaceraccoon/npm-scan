...
const getStrictExclusions = require('./utils/getStrictExclusions')
...
program
  ...
  .option('-s, --strict', 'test only high-risk heuristics')
  .parse(process.argv)
...
async function scan () {
  ...
  if (program.strict) {
    const strictExclusions = getStrictExclusions()
    console.log("Running in strict mode, excluding the following heuristics: " + strictExclusions)
    // console.log(strictExclusions)
    heuristics.file = heuristics.file.filter(heuristic => !strictExclusions.includes(heuristic.name))
    heuristics.manifest = heuristics.file.filter(heuristic => !strictExclusions.includes(heuristic.name))
  }
