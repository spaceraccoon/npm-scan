...
program
  ...
  .option('-s, --strict', 'include low-risk heuristics')
  .parse(process.argv)
...
async function scan () {
  ...
  if (!program.strict) {
    const strictHeuristics = ['lastUpdated', 'mainMinified']
    heuristics.file = heuristics.file.filter(heuristic => !strictHeuristics.includes(heuristic.name))
    heuristics.manifest = heuristics.file.filter(heuristic => !strictHeuristics.includes(heuristic.name))
  }
