...
module.exports = function printResults (results) {
  let totalVulns = 0
  let lowVulns = 0
  let medVulns = 0
  let highVulns = 0
  for (const packageName in results) {
    if (results[packageName].length !== 0) {
      ...
      console.log(`\n\x1b[31m${packageVulns}\x1b[0m vulnerabilit${packageVulns === 1 ? 'ies' : 'y'} found in ${packageName}\x1b[0m`)
  ...
  console.log(`\nFound \x1b[31m${totalVulns}\x1b[0m vulnerabilit${totalVulns === 1 ? 'ies' : 'y'} (${lowVulns} low, ${medVulns} \x1b[33mmedium\x1b[0m, ${highVulns} \x1b[31mhigh\x1b[0m) in ${Object.keys(results).length} scanned packages.`)
}
