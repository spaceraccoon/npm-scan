...
function printResults (results, verbose) {
  ...
  for (const packageName in results) {
    if (results[packageName].length !== 0) {
      ...
      console.log(`\x1b[31m${packageVulns}\x1b[0m suspicious item${packageVulns === 1 ? '' : 's'} found in ${packageName}\x1b[0m`)

      if (verbose) {
        for (let i = 0; i < packageVulns; i++) {
  ...
  console.log(`Found ${vulnStr} suspicious item${totalVulns === 1 ? '' : 's'} in ${Object.keys(results).length} scanned packages.`)
}
